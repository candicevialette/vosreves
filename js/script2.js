...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Logique pour la carte du monde interactive (uniquement sur la page carte.html)
    if (document.querySelector('.map-container')) {
        
        // Base de données des destinations avec leurs coordonnées en pourcentage (x, y)
        const destinations = {
        ...
        // Boucle pour créer et positionner chaque point
        for (const id in destinations) {
            const dest = destinations[id];
            const dot = document.createElement('div');
            
            dot.classList.add('location-dot');
            // C'est ici que le JavaScript positionne le point
...
Dans ce fichier, j'ai **supprimé toutes les lignes de positionnement des points** (celles avec `[data-destination="..."]`). Le JavaScript s'en occupe maintenant, ce qui rend le CSS plus propre.

```css
/* ... (Conservez tous les styles précédents pour le header, footer, etc.) ... */
...
.map-container {
    position: relative;
    width: 100%;
    padding-top: 56.25%; /* Ratio 16:9 pour le conteneur */
...
/* Effet au survol et quand le point est actif */
.location-dot:hover, .location-dot.active {
    transform: translate(-50%, -50%) scale(1.5);
    background-color: #e67e22;
}

/* --- TOUTES LES ANCIENNES RÈGLES DE POSITIONNEMENT ONT ÉTÉ SUPPRIMÉES D'ICI --- */

.destination-info-panel { height: 100%; }
.info-content { opacity: 0; transition: opacity 0.5s ease; visibility: hidden; }
.info-content.active { opacity: 1; visibility: visible; }
.destination-image { width: 100%; height: auto; aspect-ratio: 4 / 3; object-fit: cover; border-radius: 10px; margin-bottom: 20px; }
.info-content h3 { color: var(--primary-color); margin-bottom: 10px; font-weight: 700; }

/* ... (Le reste du CSS, notamment pour le responsive, reste identique) ... */
